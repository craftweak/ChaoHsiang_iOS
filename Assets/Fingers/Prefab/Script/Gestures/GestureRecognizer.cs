...
#pragma warning restore 0618
        ...
        /// <summary>
        /// Force a touch to be removed from tracked touches. This is not needed unless you are hacking around bugs (i.e. Unity).
        /// </summary>
        /// <param name="touch">Tracked touch to remove</param>
        public void RemoveTrackedTouch(ref GestureTouch touch)
        {
            if (currentTrackedTouches.Count != 0)
            {
                currentTrackedTouches.Remove(touch);
                if (currentTrackedTouches.Count == 0)
                {
                    FailGestureNow();
                }
